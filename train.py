...
def get_model_and_auxiliaries(args):
    ...
    # _, _, feature_extractor = open_clip.create_model_and_transforms('ViT-B-32')
    
    tokenizer = AutoTokenizer.from_pretrained(args.decoder_name)
    tokenizer.pad_token = PAD_TOKEN
    tokenizer.eos_token = EOS_TOKEN

    model = SmallCap.from_encoder_decoder_pretrained(args.encoder_name, args.decoder_name, use_channel_attn=True, use_spatial_attn=True, cross_attention_reduce_factor=cross_attention_reduce_factor)
...
def get_data(tokenizer, max_length, args):
    ...
    data = load_data_for_training(args.annotations_path, args.captions_path, class_path="data/retrieved_class.json")
