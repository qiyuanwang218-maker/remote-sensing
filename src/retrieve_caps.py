    ...
    for item in annotations:
        ...
        if item['split'] == 'train':
            for sentence in item['sentences']:
                captions.append({'image_id': item['imgid'],  'caption': ' '.join(sentence['tokens'])})
        images.append({'image_id': item['imgid'], 'file_name': item['filename']})
...
def encode_captions(captions, model, device):
    ...
    for idx in tqdm(range(0, len(captions), bs)):
        with torch.no_grad():
            input_ids = open_clip.tokenize(captions[idx:idx+bs]).to(device)
...
def encode_images(images, image_path, model, preprocess, device):
    ...
    for idx in tqdm(range(0, len(images), bs)):
        image_input = [preprocess(Image.open(os.path.join(image_path, i['file_name'])))
...
def main(): 
    ...
    encoded_captions = encode_captions(captions, model, device)
    
    print('Encoding images')
    xq_image_ids, encoded_images = encode_images(images, image_path, model, preprocess, device)
    ...
    json.dump(retrieved_caps, open('data/retrieved_caps_vit_b.json', 'w'))
