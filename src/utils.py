...
def prep_strings(text, tokenizer, template=None, retrieved_caps=None, k=None, is_test=False, max_length=None, category=None):
    ...
    if category is not None:
        prefix = f"There are {category} in the image."
    else:
        prefix = ""
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # !!!
        category = self.df.get('category', [None])[idx]  # Get category if available
        if self.rag: 
            caps = self.df['caps'][idx]
            decoder_input_ids, labels = prep_strings(text, self.tokenizer, template=self.template,
                                                     retrieved_caps=caps, k=self.k, max_length=self.max_target_length, category=category)
        else:
            decoder_input_ids, labels = prep_strings(text, self.tokenizer, max_length=self.max_target_length, category=category)
        # load precomputed features
        encoder_outputs = self.features[self.df['imgid'][idx]][()]
...
def load_data_for_training(annot_path, caps_path=None, class_path=None):
    ...
    for item in annotations:
        ...
        if caps_path is not None:
            caps = retrieved_caps[str(item['imgid'])]
        else:
            caps = None
        # £¡£¡
        categories = image_classes.get(str(item['imgid']), ["unknown"])
        ...
        for sentence in item['sentences']:
            samples.append({
                ...
                'caps': caps, 'text': ' '.join(sentence['tokens']),
                'category':category})
...
def load_data_for_inference(annot_path, caps_path=None):
    ...
    for item in annotations:
        ...
        if caps_path is not None:
            ...
            caps = None
        image = {'file_name': file_name, 'caps': caps, 'image_id': str(item['imgid'])}
        if item['split'] == 'test':
